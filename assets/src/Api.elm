...
import Json.Decode as Decode exposing (Decoder, Value, decodeString, field, list, string)
...
type Msg
    = GotGet (Result Http.Error (List String))
    | GotPost (Result Http.Error (List String))
    | GotPut (Result Http.Error (List String))
    | GotDelete (Result Http.Error (List String))
...
get : Endpoint -> Maybe Cred -> Decoder String -> Cmd Msg
get url maybeCred decoder =
    Endpoint.request
        { method = "GET"
        , url = url
        , expect = Http.expectJson GotGet (list string)
        , headers =
            case maybeCred of
                Just cred ->
                    [ credHeader cred ]
...
put : Endpoint -> Cred -> Body -> Decoder String -> Cmd Msg
put url cred body decoder =
    Endpoint.request
        { method = "PUT"
        , url = url
        , expect = Http.expectJson GotPut (list string)
...
post : Endpoint -> Maybe Cred -> Body -> Decoder a -> Cmd Msg
post url maybeCred body decoder =
    Endpoint.request
        { method = "POST"
        , url = url
        , expect = Http.expectJson GotPost (list string)
        , headers =
            case maybeCred of
                Just cred ->
                    [ credHeader cred ]
...
delete : Endpoint -> Cred -> Body -> Decoder String -> Cmd Msg
delete url cred body decoder =
    Endpoint.request
        { method = "DELETE"
        , url = url
        , expect = Http.expectJson GotDelete (list string)
        , headers = [ credHeader cred ]
        , body = body
        , timeout = Nothing
        , tracker = Nothing
